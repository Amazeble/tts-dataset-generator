...
def transcribe_audio_files(audio_dir, output_csv_path="metadata.csv", ljspeech=True, model_name="deepdml/faster-whisper-large-v3-turbo-ct2", language_="en"):
    ...
    Enforces int8_float16 precision, word-timestamps, and disables VAD filtering to protect snap sounds.
    """
    ...
    for i, filename in enumerate(wav_files):
        ...
        try:
            ...
            # Execution Step B: Run Faster-Whisper Inference with word timestamps enabled and VAD DISABLED
            segments, info = model.transcribe(
                ...
                vad_filter=False  # CRITICAL SPEED & ACCURACY FIX: Stops audio timeline mutation
            )
            
            # Extract all words into a stable list immediately to prevent generator alignment bugs
            all_words = []
            ...
            if all_words:
                for word_obj in all_words:
                    # Clear out any snaps that happened physically before the current word
                    while snap_timestamps and snap_timestamps[0] < word_obj.start:
                        final_text_pieces.append("<snap>")
                        snap_timestamps.pop(0)
                    
                    final_text_pieces.append(word_obj.word.strip())
            else:
                # Fallback if no individual words were generated by the model
                for segment in segments:
                    final_text_pieces.extend(segment.text.strip().split())
            
            # Catch trailing items at the end of the timeline past the final spoken word
            while snap_timestamps:
                final_text_pieces.append("<snap>")
                snap_timestamps.pop(0)
